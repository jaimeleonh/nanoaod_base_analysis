...
# version of law_jobs.sh at commit https://github.com/riga/law/blob/1be7657250850df5223a1a71bfa1803557149a40/law/job/law_job.sh
# when using tasks-per-job, this will run all branches in one single law run call, which with force_multiprocessing=false (the default) will not use fork with workers=1
# this allows to not re-JIT-compile for each task, but rather only once per job
# next commit of law changes it to run a fresh "law run" for each branch
