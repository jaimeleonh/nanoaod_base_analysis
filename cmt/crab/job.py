...
"""
crab job manager. See https://research.cs.wisc.edu/crab.
"""

__all__ = ["crabJobManager", "crabJobFileFactory"]
...
class crabJobManager(BaseJobManager):
    ...
    # chunking settings
    chunk_size_submit = 0
    chunk_size_cancel = _cfg.get_expanded_int("job", "crab_chunk_size_cancel")
    chunk_size_query = _cfg.get_expanded_int("job", "crab_chunk_size_query")
    ...
    def __init__(self, pool=None, scheduler=None, user=None, threads=1):
        ...
        # determine the crab version once
        self.crab_version = self.get_crab_version()

        # flags for versions with some important changes
        self.crab_v833 = self.crab_version and self.crab_version >= (8, 3, 3)
        self.crab_v856 = self.crab_version and self.crab_version >= (8, 5, 6)

    @classmethod
    def get_crab_version(cls):
    ...
    def cleanup(self, *args, **kwargs):
        raise NotImplementedError("crabJobManager.cleanup is not implemented")

    def cleanup_batch(self, *args, **kwargs):
        raise NotImplementedError("crabJobManager.cleanup_batch is not implemented")
    ...
    def submit(self, job_file, pool=None, scheduler=None, retries=0, retry_delay=3, silent=False):
        ...
        while True:
            # run the command
            logger.debug("submit crab job with command '{}'".format(cmd))
    ...
    def query(self, job_id, pool=None, scheduler=None, user=None, silent=False):
        ...
        if code != 0:
            if silent:
                ...
                raise Exception("queue query of crab job(s) '{}' failed with code {}:"
        ...
        if missing_ids:
            ...
            logger.debug("query crab job history with command '{}'".format(cmd))
            code, out, err = interruptable_popen(cmd, shell=True, executable="/bin/bash",
                stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            ...
            if code != 0:
                if silent:
                    ...
                    raise Exception("history query of crab job(s) '{}' failed with code {}:"
                        "\n{}".format(job_id, code, err))
            ...
            if _job_id not in query_data:
                if not chunking:
                    if silent:
                        ...
                        raise Exception("crab job(s) '{}' not found in query response".format(
                            job_id))
                else:
